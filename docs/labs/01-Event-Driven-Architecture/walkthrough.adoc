...
. Open the OpenShift Developer Console link:{openshift-host}/topology/ns/{namespace}[Topology View, window="_blank"], and link the recently created secret values with as environment variables for the `events` Deployment.
+
image::images/secret-deployment-add.png[Environment Variables, role="integr8ly-img-responsive"]
